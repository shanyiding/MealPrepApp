...
class MealCard(Card):
    ...
    def __init__(
        ...
        meal_number,
        ingredient_names,
        remove_card_callback=None,
        prefill_name="",
        prefill_ingredients=None,
        **kwargs,
    ):
        super().__init__(
    ...
    def _build(self, prefill_name, prefill_ingredients):
        ...
        if self.remove_card_callback:
            delete_btn = IconBtn("x", bg_color=DEL_BG, text_color=DEL_TXT)
            delete_btn.bind(on_press=lambda _: self.remove_card_callback(self))
        ...
        if prefill_ingredients:
            for item in prefill_ingredients:
                if isinstance(item, dict):
                    ...
                    amount = ""
                    if len(parts) > 1:
                        amount = parts[1].strip().rstrip("abcdefghijklmnopqrstuvwxyz ")
                    self._add_row(name=name, amount=amount)
        else:
            self._add_row()
    ...
    def _resize(self):
        n = len(self.rows)
        self.ingredients_box.height = (
            n * self.ROW_H + max(0, n - 1) * self.ROW_GAP
        )
        ...
        **kwargs,
        ...
        for line in meal_data["ingredients"]:
            ...
            if len(parts) == 2:
                display = (
                    f"[b]{parts[0].strip()}[/b]  "
                    f"[color=#849096]{parts[1].strip()}[/color]"
                )
            else:
                display = line
...
class DailyTrackerUI(BoxLayout):
    ...
    def _build_summary(self):
        ...
        self._cal_cap = Label(
            text=f"Goal {theme.cal_goal} kcal",
            font_size=10,
            color=MUTED,
            halign="left",
            valign="middle",
        ...
        self._pro_cap = Label(
            text=f"Goal {theme.protein_goal}g protein",
            font_size=10,
            color=MUTED,
            halign="right",
            valign="middle",
    ...
    def _on_goals_changed(self, *_):
        ...
        self._cal_bar.set_progress(self._last_cal, theme.cal_goal)
        self._pro_bar.set_progress(self._last_pro, theme.protein_goal)
        self._comment.text = self._daily_comment(self._last_cal, self._last_pro)
    ...
    def add_meal_card(self):
        ...
        card = MealCard(
            ...
            ingredient_names=self.ingredient_names,
            remove_card_callback=self.remove_unsaved_meal_card,
        )

        self.meal_box.add_widget(card)

    def remove_unsaved_meal_card(self, card):
        if card.parent:
            self.meal_box.remove_widget(card)
    ...
    def _on_edit_meal(self, meal_name, meal_data):
        ...
        edit_card._editing_meal_name = meal_name
